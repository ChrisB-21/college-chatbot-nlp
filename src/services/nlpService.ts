...
export const initializeModels = async () => {
  try {
    ...
    // Try to load with a more compatible model
    try {
      ...
      // Store the error for the diagnostic component to display
      ...
      // Try alternative model source or fall back to CPU
      try {
    ...
    sessionStorage.setItem('model_load_error', 
      error instanceof Error 
        ? error.message 
        : 'Unknown error initializing models');
    return false;
  }
};
